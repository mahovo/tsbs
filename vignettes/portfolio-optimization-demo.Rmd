...
```{r global_settings}
## =============================================================================
## GLOBAL SETTINGS - Adjust these to control computation time
## =============================================================================
...
## Function to summarize bootstrap weights
...
## Compare models for first rebalance
...
The **tsbs** package provides comprehensive diagnostics for bootstrap analysis. Here we examine the diagnostics for each model type.
...
## Function to extract and print key diagnostics
print_diagnostics_summary <- function(diag_obj, model_name) {
  if (is.null(diag_obj)) {
    cat("\n", model_name, ": No diagnostics available\n")
    return(invisible(NULL))
  }
  
  cat("\n--- ", model_name, " Diagnostics ---\n", sep = "")
  
  ## Check if this is a tsbs_diagnostics object
  if (inherits(diag_obj, "tsbs_diagnostics")) {
    summary(diag_obj)
  } else if (is.list(diag_obj)) {
    ## Try to extract relevant information
    if (!is.null(diag_obj$meta)) {
      cat("Bootstrap type:", diag_obj$meta$bs_type, "\n")
      cat("Replicates:", diag_obj$meta$num_boots, "\n")
      cat("Series length:", diag_obj$meta$n_original, "\n")
    }
    
    ## Print any convergence info
    if (!is.null(diag_obj$convergence)) {
      cat("Convergence status:", diag_obj$convergence, "\n")
    }
    
    if (!is.null(diag_obj$iterations)) {
      cat("Iterations:", diag_obj$iterations, "\n")
    }
  }
}

## Print diagnostics for each model from first rebalance
print_diagnostics_summary(safe_get(diagnostics_dcc, 1), "DCC")
print_diagnostics_summary(safe_get(diagnostics_cgarch, 1), "CGARCH")
print_diagnostics_summary(safe_get(diagnostics_gogarch, 1), "GOGARCH")
```
...
## Extract bootstrap statistics if available
...
## Function to compute robust estimates
compute_robust_estimates <- function(boot_w, point_est, symbols) {
  if (is.null(boot_w)) return(NULL)
  if (!is.matrix(boot_w) && !is.data.frame(boot_w)) return(NULL)
  if (nrow(boot_w) < 2) return(NULL)
...
if (!is.null(dcc_boot)) {
  robust_results[["DCC"]] <- compute_robust_estimates(dcc_boot, point_est, symbols)
}

cgarch_boot <- safe_get(boot_results_cgarch, 1)
if (!is.null(cgarch_boot)) {
  robust_results[["CGARCH"]] <- compute_robust_estimates(cgarch_boot, point_est, symbols)
}

gogarch_boot <- safe_get(boot_results_gogarch, 1)
if (!is.null(gogarch_boot)) {
  robust_results[["GOGARCH"]] <- compute_robust_estimates(gogarch_boot, point_est, symbols)
}
