...
### ADCC (Asymmetric DCC) Considerations
...
$$Q_t = (1 - \alpha - \beta - \delta\bar{\gamma})\bar{Q} + \alpha z_{t-1}z_{t-1}' + \beta Q_{t-1} + \gamma n_{t-1}n_{t-1}'$$

where $n_t = z_t \cdot \mathbf{1}(z_t < 0)$ captures negative shocks.

**Inference considerations for ADCC**:
- The gamma parameter may have identification issues when asymmetric effects are weak
- Bootstrap is strongly recommended for all three parameters
- Profile likelihood can help assess parameter identifiability
