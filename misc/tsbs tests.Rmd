...
```{r}
#' Advanced Visualization for MS-VAR Model and Bootstrap Results
#'
#' This script demonstrates how to visualize the output of an MS-VAR model
...
set.seed(123)
T_obs <- 100
num_resampled_blocks <- 20 # Number of blocks to build the new series
y1 <- arima.sim(model = list(ar = 0.7), n = T_obs)
#y2 <- 0.5 * y1 + arima.sim(model = list(ar = 0.5), n = T_obs)
y2 <- arima.sim(model = list(ar = 0.1), n = T_obs)
...
## --- 3. Manually Perform One Bootstrap to Track States ---
...
plot_with_states <- function(series, states, title) {
  ...
  # Add vertical lines at the start of each block
  abline(v = state_starts, col = "red", lty = 2, lwd = 1.5)
  ...
  legend(
    ...
    pt.cex = c(2.5, 2.5, 1),  # Size of the legend points
...
plot_with_states(
  ...
  title = "Original Series (y1) with Estimated States"
)
...
plot_with_states(
  ...
  title = "Bootstrapped Series (y1) with Reconstructed States"
)
