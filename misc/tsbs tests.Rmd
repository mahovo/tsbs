...
### Example 1a: Basic Diagnostic Collection
...
### Example 1b: Write verbose output 
```{r}
fit <- fit_ms_varma_garch(
  ...
  spec = spec_test,
  model_type = "multivariate",
  control = list(max_iter = 2, tol = 0.2),
  collect_diagnostics = TRUE,
  verbose = TRUE,
  verbose_file = "my_estimation_log.txt"  ## Goes to file
)

## Then examine the file:
readLines("my_estimation_log.txt") |> head(50)
```
...
dcc_spec <- generate_dcc_spec(M = 2, k = 2, seed = 999)

dcc_spec_obj <- tsgarch::garch_modelspec(
      y = residuals_xts,
...
dcc_spec_obj$parmatrix[group %in% c("alpha", "beta", "gamma"), 
                      c("parameter", "lower", "upper", "estimate")]
```
