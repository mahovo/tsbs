...
max_num_rb = 3 ## Maximum number of rebalances
max_iter = 2 ## Maximum EM iterations
```
...
```{r chunk2, echo=FALSE, results=FALSE, message=FALSE}
library(tsbs)
library(quantmod)
```
...
risk_parity_portfolio <- function(returns, ...) {
  ...
  ## Defensive: check for valid covariance matrix
  vols <- sqrt(diag(Sigma))
  ...
  ## Initial weights proportional to 1/sigma_i
  weights <- 1 / vols
  weights <- weights / sum(weights)
  ...
  for (iter in 1:20) {
    ...
    target_rc <- sum(rc) / n
    adjustment <- target_rc / (rc + 1e-8)
    weights <- weights * sqrt(pmax(adjustment, 0))  # Prevent sqrt of negative
...
```{r chunk5, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
set.seed(42)
...
## Capture diagnostics
if(collect_diagnostics && return_fit) {diagnostics <- list()}

## Storage for bootstrap uncertainty
boot_weight_history <- list()
if(collect_diagnostics && return_fit) {diagnostics_history <- list()}
...
for (rb_idx in seq_along(rebalance_dates)) {
  ...
  ## Bootstrap for Risk Parity to get uncertainty estimates
  if (rb_idx <= max_num_rb) {
    result <- tryCatch({
      ...
      list(
        ...
        diagnostics = if(collect_diagnostics && return_fit) {
          boot_result$fit$diagnostics
        } else {NULL}
      )
    }, error = function(e) {
    ...
    if (!is.null(result)) {
      ...
      if(collect_diagnostics && return_fit) {
...
for (rb_idx in 1:min(3, length(boot_weight_history))) {
  ...
  if (!is.null(boot_w) && nrow(boot_w) > 1) {
    ...
    if (ncol(boot_w) == length(symbols)) {
      ...
      warning(sprintf("Rebalance %d: boot_w has %d columns but symbols has %d elements", 
                      rb_idx, ncol(boot_w), length(symbols)))
      next
    }
...
## Plot 3b: Weight evolution (Risk Parity)
w_hist <- weight_history[["Max Sharpe"]]
w_hist <- w_hist[complete.cases(w_hist), ]
if (nrow(w_hist) > 1) {
  barplot(t(w_hist), col = rainbow(k), border = NA,
          main = "Sharpe Weights Over Time",
          xlab = "Rebalance Period", ylab = "Weight")
  legend("topright", symbols, fill = rainbow(k), cex = 0.7, ncol = 2)
}
```
...
## Plot 3b: Weight evolution (Risk Parity)
w_hist <- weight_history[["Risk Parity"]]
w_hist <- w_hist[complete.cases(w_hist), ]
if (nrow(w_hist) > 1) {
  barplot(t(w_hist), col = rainbow(k), border = NA,
          main = "Risk Parity Weights Over Time",
          xlab = "Rebalance Period", ylab = "Weight")
  legend("topright", symbols, fill = rainbow(k), cex = 0.7, ncol = 2)
}
```
...
## Plot 3b: Weight evolution (Risk Parity)
w_hist <- weight_history[["Black-Litterman"]]
w_hist <- w_hist[complete.cases(w_hist), ]
if (nrow(w_hist) > 1) {
  barplot(t(w_hist), col = rainbow(k), border = NA,
          main = "Black-Litterman Weights Over Time",
          xlab = "Rebalance Period", ylab = "Weight")
  legend("topright", symbols, fill = rainbow(k), cex = 0.7, ncol = 2)
}
```
...
## Plot 3b: Weight evolution (Risk Parity)
w_hist <- weight_history[["Shrinkage"]]
w_hist <- w_hist[complete.cases(w_hist), ]
if (nrow(w_hist) > 1) {
  barplot(t(w_hist), col = rainbow(k), border = NA,
          main = "Shrinkage Weights Over Time",
          xlab = "Rebalance Period", ylab = "Weight")
  legend("topright", symbols, fill = rainbow(k), cex = 0.7, ncol = 2)
}
```
...
if (!is.null(boot_weight_history[[1]])) {
  ...
  boxplot(boot_w, col = rainbow(k),
          main = "Risk Parity Bootstrap Weight Distribution\n(First Rebalance)",
          xlab = "Asset", ylab = "Weight",
          names = symbols)
  points(1:k, weight_history[["Risk Parity"]][1, ], pch = 18, col = "black", cex = 1.5)
}
