...
This vignette provides guidance on statistical inference for correlation
parameters in the tsbs package’s multivariate GARCH models. The package
supports three correlation model specifications:
...
Each model has distinct inference characteristics that require different
approaches for reliable standard errors and confidence intervals.
...
**Key Finding**: Both DCC and CGARCH share the “flat beta problem”—the
likelihood surface is approximately 8x flatter in the beta direction
than the alpha direction. This causes Hessian-based standard errors to
severely underestimate uncertainty for beta. GOGARCH has a different
structure but benefits from bootstrap inference due to ICA estimation
uncertainty.
...
The DCC(1,1) model has parameters α (news impact) and β (persistence).
Validation studies reveal that the likelihood surface has highly
...
The “flat direction” eigenvector is approximately (0.25, -0.97), meaning
the likelihood is nearly constant along lines in parameter space. This
is a **fundamental property of DCC with high persistence**, not an
implementation error.

### DCC Standard Error Calibration

Monte Carlo studies reveal severe miscalibration of Hessian-based SEs
for beta:
...
**Interpretation**: Alpha’s Hessian SE is ~80% of empirical
SD—acceptable for inference. Beta’s Hessian SE is only ~16% of bootstrap
SE—severely underestimated.
...
- `"parametric"`: Simulates new data from fitted DCC model (more
...
structure. The key difference from DCC is the Probability Integral
Transform (PIT) applied to standardized residuals before modeling
correlation dynamics.

### CGARCH Model Components

1.  **Marginal Transformation**: Standardized residuals → Uniform
    \[0,1\] via PIT
...
CGARCH shares the same “flat beta problem” as DCC because the
correlation dynamics follow the same DCC(1,1) recursion. The copula
transformation does not change the fundamental shape of the likelihood
surface with respect to (α, β).
...
ADCC adds a leverage parameter γ that captures asymmetric correlation
responses to positive vs. negative shocks. The ADCC recursion is:

$$Q_{t} = \Omega + \alpha z_{t - 1}z_{t - 1}\prime + \gamma n_{t - 1}n_{t - 1}\prime + \beta Q_{t - 1}$$

where: - $n_{t} = z_{t} \cdot \mathbf{1}\left( z_{t} < 0 \right)$
captures negative shocks -
$\bar{N} = E\left\lbrack n_{t}n_{t}\prime \right\rbrack$ is the expected
outer product of negative shocks -
$\Omega = (1 - \alpha - \beta)\bar{Q} - \gamma\bar{N}$ ensures
stationarity - Stationarity requires:
$\alpha + \beta + \delta\gamma < 1$ where $\delta \approx 0.5$

#### ADCC Hessian-Based Standard Errors

The
[`adcc_standard_errors()`](https://mahovo.github.io/tsbs/reference/adcc_standard_errors.md)
function computes Hessian-based SEs for the 3-parameter ADCC model:
...
For production use, the robust wrapper handles boundary estimates and
numerical issues:
...
As with DCC/CGARCH, bootstrap is recommended for reliable inference,
especially for beta:
...
1.  **Gamma identification**: When asymmetric effects are weak, γ may be
    poorly identified. Check the Hessian eigenvalues for
    near-singularity.

2.  **Stationarity boundary**: The constraint α + β + δγ \< 1 means
    high-persistence models have less room for asymmetry.

3.  **Bootstrap coverage**: Monte Carlo studies show bootstrap provides
    better coverage than Hessian for all three parameters.
...
> The ADCC parameters were estimated as α̂ = 0.045 (SE = 0.022), γ̂ =
> 0.028 (bootstrap 95% CI: \[0.003, 0.061\]), and β̂ = 0.91 (bootstrap
> 95% CI: \[0.52, 0.97\]). The asymmetric parameter γ is statistically
> significant, indicating stronger correlation increases following joint
> negative returns. Bootstrap confidence intervals are reported for γ
> and β due to potential identification issues with Hessian-based
> inference.
...
Generalized Orthogonal GARCH (GOGARCH) models multivariate volatility
through Independent Component Analysis (ICA). Unlike DCC/CGARCH, which
model correlation dynamics directly, GOGARCH:

1.  Decomposes residuals into independent components via ICA
2.  Fits univariate GARCH to each independent component
3.  Derives time-varying correlation from the mixing matrix and
...
1.  **ICA estimation uncertainty**: The mixing matrix W is estimated,
    but this uncertainty is not captured by standard Hessian-based SEs
    for the GARCH parameters.

2.  **Two-stage estimation**: ICA is performed first, then GARCH is
    estimated on the components. Standard errors that only consider the
    second stage are incomplete.

3.  **Component independence assumption**: Bootstrap naturally
    propagates uncertainty through both stages.
...
**Note**: These SEs do not account for ICA estimation uncertainty and
...
The bootstrap procedure: 1. Resamples the independent components
(preserving independence) 2. Re-estimates GARCH for each component 3.
Computes empirical standard deviations
...
**Note**: Profile likelihood is not available for GOGARCH due to the ICA
structure. The bootstrap is the primary recommended method.

------------------------------------------------------------------------

## Part IV: Unified Inference Interface

The tsbs package provides unified functions that work across all three
model types.
...
All three models can produce boundary estimates that invalidate standard
inference.
...
When persistence is high in DCC/CGARCH, the likelihood becomes
increasingly flat.

**Recommendations**: - Always use bootstrap or profile likelihood for
beta - Consider reporting persistence rather than individual
parameters - Be cautious about point estimates—they may not be
well-identified
...
**Recommendations**: - Increase bootstrap replications (n_boot ≥ 500) -
Consider wider confidence intervals - Report multiple inference methods
for comparison
...
**Our implementation**: - Detects boundary estimates and returns NA for
SEs - Profile likelihood handles boundaries correctly - Bootstrap may
produce boundary estimates in some replications

### GOGARCH-Specific Issues

**ICA sensitivity**: Results can depend on ICA initialization and
method.

**Recommendations**: - Consider multiple ICA runs with different seeds -
Bootstrap captures some of this variation - Report ICA method used
(e.g., “fastica”, “jade”)
...
> (bootstrap 95% CI: \[0.55, 0.96\]). Hessian-based standard errors for
> β are known to underestimate uncertainty for high-persistence DCC
> models; bootstrap confidence intervals are reported instead.
...
**DCC Models**: - Engle, R. (2002). Dynamic conditional correlation: A
...
20(3), 339-350. - Aielli, G. P. (2013). Dynamic conditional correlation:
On properties and estimation. *Journal of Business & Economic
Statistics*, 31(3), 282-299.

**Copula GARCH**: - Patton, A. J. (2006). Modelling asymmetric exchange
rate dependence. *International Economic Review*, 47(2), 527-556. -
Jondeau, E. & Rockinger, M. (2006). The Copula-GARCH model of
conditional dependencies. *Journal of International Money and Finance*,
25(5), 827-853.

**GOGARCH**: - van der Weide, R. (2002). GO-GARCH: A multivariate
generalized orthogonal GARCH model. *Journal of Applied Econometrics*,
17(5), 549-564. - Boswijk, H. P. & van der Weide, R. (2011). Method of
moments estimation of GO-GARCH models. *Journal of Econometrics*,
163(1), 118-126.

**Software**: - tsmarch package: <https://github.com/tsmodels/tsmarch> -
